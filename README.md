...
- **`_chapters/` working folder:** while the script runs, each chapter is written as a WAV to `<out-dir>/_chapters/` so you can see progress. The folder is auto-deleted once the final output is produced. If the run crashes or is interrupted, your completed chapters stay there — useful for manual recovery or resuming with `ffmpeg`.
